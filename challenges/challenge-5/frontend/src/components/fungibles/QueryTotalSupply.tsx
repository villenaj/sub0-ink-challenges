...
export default function QueryTotalSupply(props: Props) {
  const { contract } = useContract<FungiblesContractApi>(ContractId.FUNGIBLES);
  const [assetId, setAssetId] = useState<number>(0);
  const query = useContractQuery({ contract, args: [assetId || 0], fn: 'totalSupply' });

  return (
    <Flex alignItems={'center'}>
      <ContractQueryBadge title='Total Supply' />
      <Input
        type='number'
        mx={2}
        value={assetId}
